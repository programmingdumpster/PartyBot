...
class PartyManagementCog(commands.Cog, name="Zarządzanie Party"):
    ...
    async def _update_party_emblem(self, party_id: int):
        ...
        try:
            ...
            view = disnake.ui.View(timeout=None)  # Upewnij się, że widok jest taki sam jak przy tworzeniu
            ...
            if party_data.get("category_id"):
                ...
                if category and isinstance(category, disnake.CategoryChannel):
                    ...
                    try:
                        ...
                        pass
            else:  # Jeśli nie ma kategorii, usuń kanały indywidualnie (fallback)
    ...
    async def party_command_handler(self, ctx: commands.Context):
        ...
        category_overwrites = {
            guild.default_role: disnake.PermissionOverwrite(
                view_channel=True,
                read_messages=True,  # Umożliwia widzenie kanału
                send_messages=False,  # Blokuje pisanie na głównym kanale
                connect=False,  # Dla kanałów głosowych
                speak=False,  # Dla kanałów głosowych
                create_public_threads=False,  # <-- WAŻNA ZMIANA: Blokuje tworzenie wątków publicznych
                create_private_threads=False,  # <-- WAŻNA ZMIANA: Blokuje tworzenie wątków prywatnych
                send_messages_in_threads=False
                # <-- Dodatkowo: Blokuje pisanie w wątkach dla @everyone (jeśli jakimś cudem by się tam znaleźli)
            ),
            guild.me: disnake.PermissionOverwrite(  # Uprawnienia dla bota
                ...
                create_public_threads=True,  # Bot może potrzebować, jeśli zarządza wątkami
                create_private_threads=True,
                send_messages_in_threads=True,
                manage_threads=True  # Bot może zarządzać wątkami
            ),
            leader: disnake.PermissionOverwrite(  # Uprawnienia dla lidera
                view_channel=True, read_messages=True, send_messages=True,
                connect=True, speak=True, manage_messages=True,
                mute_members=True, deafen_members=True, move_members=True,
                create_public_threads=True,  # <-- Lider MOŻE tworzyć wątki
                create_private_threads=True,  # <-- Lider MOŻE tworzyć wątki
                send_messages_in_threads=True  # Lider MOŻE pisać w wątkach
            )
        }
        category = None;
        settings_ch = None;
    ...
    async def on_button_interaction(self, interaction: disnake.MessageInteraction):
        ...
        if custom_id.startswith("request_join_party_") or custom_id.startswith("settings_request_join_"):
            ...
            party_data_check = active_parties.get(party_id)  # Użyj innej nazwy zmiennej, aby uniknąć konfliktu
    ...
    async def leave_party_dm_command(self, ctx: commands.Context, *, party_identifier: str):
        ...
        if not target_party_id_to_leave:
            found_by_name = [p_info for p_in_list in parties_member_of_and_not_leader if
                             p_in_list['name'].lower() == party_identifier.lower().strip()]  # Poprawka: p_in_list
    ...
    async def leave_party_dm_command_error_handler(self, ctx, error):  # Zmieniona nazwa error handlera
        bot_response_msg = None
